...
const errorHandler = (error, request, response, next) => {
    ...
    if (error.name === "ValidationError") {
        ...
        return response.status(HttpStatus.FORBIDDEN).send({ message: error.message });
    } else if (error instanceof Errors.NotFound) {
        return response.status(HttpStatus.NOT_FOUND).send({ message: error.message });
    } else if (error instanceof Errors.UnprocessableEntity) {
        return response
            .status(HttpStatus.UNPROCESSABLE_ENTITY)
            .send({ message: error.message });
    } else if (error instanceof Errors.Conflict) {
        return response.status(HttpStatus.CONFLICT).send({ message: error.message });
